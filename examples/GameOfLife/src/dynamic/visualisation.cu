...
GLuint cell_default_tbo;
GLuint cell_default_displacementTex;
...
__global__ void output_cell_agent_to_VBO(xmachine_memory_cell_list* agents, glm::vec4* vbo, glm::vec3 centralise, int population_width){
    ...
    vbo[index].z = 0.0;
    vbo[index].w = agents->state[index];
}
...
void initVisualisation()
{
	...
	// create TBO
	createTBO( &cell_default_tbo, &cell_default_displacementTex, xmachine_memory_cell_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_cell_default_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, cell_default_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_cell_default_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
        //discrete variables
        int population_width = (int)floor(sqrt((float)get_agent_cell_default_count()));
		centralise.x = population_width / 2.0;
        centralise.y = population_width / 2.0;
        centralise.z = 0.0;
        
		output_cell_agent_to_VBO<<< grid, threads>>>(get_device_cell_default_agents(), dptr, centralise, population_width);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(cell_default_tbo));
	}
...
void display()
{
	...
	//Draw cell Agents in default state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, cell_default_displacementTex);
	//loop
	for (int i=0; i< get_agent_cell_default_count(); i++){
